...
class BLIX_PT_stacking(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        col.prop(scene, "blix_stack_scale")
        sub = col.column()
        sub.active = props.stack_scale(scene) > 1
        sub.prop(scene, "blix_stack_scale_layers")
        col.prop(scene, "blix_stack_noise")
        sub = col.column()
...
class BLIX_PT_dither(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        if props.dither_pattern(scene) == "BLUE":
            col.prop(scene, "blix_dither_seed")
